...
function eventRow(e: LogEvent, now: number, units: Units, onSelect: (e: LogEvent) => void, nested?: boolean) {
  const Icon = ICON[iconKey(e)];
  const open = isRest(e) && e.endAt === undefined;
  const tone = eventTone(e);
  const detail = open ? `still ${e.kind === "sleep" ? "asleep" : "napping"} · ${durationLabel(e.at, now)}` : isRest(e) ? durationLabel(e.at, e.endAt!) : eventDetail(e, units);
...
export function Timeline({ items, now, onSelect }: { items: TimelineItem[]; now: number; onSelect: (e: LogEvent) => void }) {
  ...
  return (
    <ul className="space-y-2">
      {items.map((it) => {
        if (it.type === "event") return <li key={it.event.id}>{eventRow(it.event, now, units, onSelect)}</li>;
          ...
          <li key={key} className={`card-soft p-3 ${it.open ? "border-sleep/50" : ""}`}>
            ...
            {isOpen ? (
              <div className="mt-1 border-t border-border pl-1">
                {[...it.parts, ...it.inside]
                  .sort((a, b) => b.at - a.at)
                  .map((e) => eventRow(e, now, units, onSelect, true))}
              </div>
            ) : null}
          </li>
        );
